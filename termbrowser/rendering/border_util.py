...
def renderTableBorder(type: str, pos: Vec, col_widths: list[int], row_heights: list[int], res):
    screenSize = Vec(len(res.rows[0]), len(res.rows))
    borderCodes = getBorderCodes(type)

    current_y = pos.y
    total_width = sum(col_widths) + len(col_widths) + 1  # column widths + separators + outer borders
    ...
    if current_y >= 0:
        ...
        res.rows[current_y] = top_row if len(top_row) <= screenSize.x else top_row[:screenSize.x]
        current_y += 1
    ...
    for row_idx, height in enumerate(row_heights):
        ...
        for _ in range(height):
            ...
            res.rows[current_y] = content_row if len(content_row) <= screenSize.x else content_row[:screenSize.x]
            current_y += 1

        # Row separator (not after last row)
        if row_idx < len(row_heights) - 1 and current_y < len(res.rows) and current_y >= 0:
            ...
            res.rows[current_y] = separator_row if len(separator_row) <= screenSize.x else separator_row[:screenSize.x]
            current_y += 1
    ...
    if current_y < len(res.rows) and current_y >= 0:
        ...
        res.rows[current_y] = bottom_row if len(bottom_row) <= screenSize.x else bottom_row[:screenSize.x]
